...
namespace QuantConnect.ToolBox.BitstampDataDownloader
{
    public static class BitstampDownloaderProgram
    {
        /// <summary>
        /// Bitstamp Downloader Toolbox Project For LEAN Algorithmic Trading Engine.
        /// By @matthewsedam
        /// </summary>
        public static void BitstampDownloader(IList<string> tickers, string resolution, DateTime startDate, DateTime endDate)
        {
            if (resolution.IsNullOrEmpty() || tickers.IsNullOrEmpty())
            {
                Console.WriteLine("BitstampDownloader ERROR: '--tickers=' or '--resolution=' parameter is missing");
            ...
            try
            {
                ...
                var downloader = new BitstampDownloader.BitstampDataDownloader();

                foreach (var pair in tickers)
                {
                    // Download data
                    var pairObject = Symbol.Create(pair, SecurityType.Crypto, Market.Bitstamp);
        ...
        {
            new ExchangeInfoUpdater(new BitstampExchangeInfoDownloader())
                .Run();
        }
    }
}
