...
{
    public partial class BitstampBrokerageTests : BrokerageTests
    {
        ...
        protected override IBrokerage CreateBrokerage(IOrderProvider orderProvider, ISecurityProvider securityProvider)
        {
            ...
            algorithm.Setup(a => a.BrokerageModel).Returns(new BitstampBrokerageModel1(AccountType.Margin));
            algorithm.Setup(a => a.Portfolio).Returns(new SecurityPortfolioManager(securities, transactions, new AlgorithmSettings()));

            if (environment.Contains("Bitstamp_PUBLIC"))
            {
                Config.Set("Bitstamp-api-key", environment["Bitstamp_PUBLIC"].ToString());
            }
            else
            {
                Log.Error("Environment doesn't have Bitstamp_PUBLIC variable");
            }
            
            if (environment.Contains("Bitstamp_PRIVATE"))
            {
                Config.Set("Bitstamp-api-secret", environment["Bitstamp_PRIVATE"].ToString());
            }
            else
            {
                Log.Error("Environment doesn't have Bitstamp_PRIVATE variable");
            }
            
            var apiKey = Config.Get("Bitstamp-api-key");
            var apiSecret = Config.Get("Bitstamp-api-secret");
            var tier = Config.Get("Bitstamp-verification-tier");
            var orderBookDepth = Config.GetInt("Bitstamp-orderbook-depth", 10);


            return new BitstampBrokerage(apiKey, apiSecret, tier, orderBookDepth, algorithm.Object, new AggregationManager(), null);
        }

        protected override Symbol Symbol => StaticSymbol;

        private static Symbol StaticSymbol => Symbol.Create("BTCUSD", SecurityType.Crypto, Market.Bitstamp);
        ...
        {
            var brokerage = (BitstampBrokerage) Brokerage;
            var tick = brokerage.GetTick(symbol);

            return tick.AskPrice;
        }

        protected override decimal GetDefaultQuantity() => SymbolPropertiesDatabase.FromDataFolder().GetSymbolProperties(Market.Bitstamp, Symbol, Symbol.SecurityType, Currencies.USD).MinimumOrderSize.Value; // Take order minimum
        ...
        [Test, Description("In Bitstamp AccountHoldings would always return 0 as long as leverage 1")]
        ...
        private Crypto SecurityInit(out SecurityPortfolioModel model, out SecurityManager securities)
        {
            ...
            security.FeeModel = new BitstampFeeModel1();
