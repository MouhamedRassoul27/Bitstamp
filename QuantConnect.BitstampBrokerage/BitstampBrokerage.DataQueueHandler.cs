...
namespace QuantConnect.Brokerages.Bitstamp
{
    public partial class BitstampBrokerage
    {
        ...
        private readonly ConcurrentDictionary<Symbol, BitstampOrderBook> _orderBooks = new ();
        ...
        private void OnDataMessage(WebSocketMessage webSocketMessage)
        {
            ...
            if (token is JObject)
            {
                switch (token["event"].ToString())
                {
                    ...
                    case "subscriptionStatus":
                        ...
                        {
                            Log.Error($"BitstampDataQueueHandler.On{token["status"]}(): Channel {token["status"]}: Error:{token["errorMessage"]}");
                        }
                        else
                        {
                            Log.Trace($"BitstampDataQueueHandler.On{token["status"]}(): Channel {token["status"]}: Id:{token["channelID"]} {token["pair"]}/{token["channelName"]}");
                        }
                        break;
                    case "error":
                        Log.Error($"BitstampDataQueueHandler Error: {token["errorMessage"]}");
                        break;
                }
            }
            else if (token is JArray jToken)
            {
                ...
                {
                    ParseTradeMessage(symbol, jToken[1].ToObject<List<BitstampTrade>>());
                }
            }
        }
        ...
        private void ParseTradeMessage(Symbol symbol, List<BitstampTrade> trades)
        {
        ...
        private void ProcessOrderBookSnapshot(Symbol symbol, JToken book)
        {
            try
            {
                ...
                lock(orderBook)
                {
                    ...
                    {
                        var bidAsk = entry.ToObject<BitstampBidAsk>();
            ...
            try
            {
                ...
                lock(orderBook)
                {
                    if (book["a"] != null)
                    {
                        ...
                        {
                            var ask = rawAsk.ToObject<BitstampBidAsk>();
                            if (ask.Volume == 0)
                            {
                    ...
                    if (book["b"] != null)
                    {
                        ...
                        {
                            var bid = rawBid.ToObject<BitstampBidAsk>();
                            if (bid.Volume == 0)
                            {
        ...
        {
            if (symbol.Value.Contains("UNIVERSE") || !_symbolMapper.IsKnownLeanSymbol(symbol) || symbol.ID.Market != Market.Bitstamp)
            {
                return false;
            }
            return true;
        ...
        public void SetJob(LiveNodePacket job)
        {
            ...
            {
                throw new ArgumentException($"BitstampBrokerage.SetJob(): missing value -- Bitstamp-orderbook-depth");
            }
