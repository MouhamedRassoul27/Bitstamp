...
{
    /// <summary>
    /// Due to Bitstamp weirdness with symbols, this override helps to solve them all.
    /// Very close to <see cref="SymbolPropertiesDatabaseSymbolMapper"/>, but with Bitstamp tweaks
    /// </summary>
    public class BitstampSymbolMapper : ISymbolMapper
    {
        ...
        public BitstampSymbolMapper()
        {
            _symbolPropertiesMap =
                SymbolPropertiesDatabase
                    .FromDataFolder()
                    .GetSymbolPropertiesList(Market.Bitstamp)
        ...
        /// <summary>
        /// Get Bitstamp Market ticker for passed <see cref="Symbol"/>
        /// </summary>
        ...
        {
            if (symbol.ID.Market != Market.Bitstamp)
            {
                throw new ArgumentException($"This method applies only for Bitstamp symbols. Try to use other class like {nameof(SymbolPropertiesDatabaseSymbolMapper)}");
            }
            
            if (symbol.SecurityType != SecurityType.Crypto)
            {
                throw new ArgumentException($"Only crypto symbols available in Bitstamp now. Current symbol security type: {symbol.SecurityType}");
            }
        ...
        /// <summary>
        /// Return Lean <see cref="Symbol"/> instance for passed Bitstamp market Ticker
        /// </summary>
        /// <param name="brokerageSymbol">Symbol that comes from Bitstamp API</param>
        /// <param name="securityType">Always Crypto</param>
        /// <param name="market">Always Bitstamp</param>
        ...
        /// <exception cref="ArgumentException">Wrong Lean <see cref="Symbol"/></exception>
        public Symbol GetLeanSymbol(string brokerageSymbol, SecurityType securityType = SecurityType.Crypto, string market = Market.Bitstamp, DateTime expirationDate = default(DateTime),
            decimal strike = 0, OptionRight optionRight = OptionRight.Call)
        {
            if (market != Market.Bitstamp)
            {
                throw new ArgumentException($"This method applies only for Bitstamp symbols. Try to use other class like {nameof(SymbolPropertiesDatabaseSymbolMapper)}");
            }
            ...
            {
                throw new ArgumentException($"Only crypto symbols available in Bitstamp now. Current symbol security type: {securityType}");
            }
        ...
        /// <summary>
        /// Return Lean <see cref="Symbol"/> instance for passed Bitstamp market Ticker from OpenOrders endpoint
        /// </summary>
        ...
        /// <returns>Lean <see cref="Symbol"/></returns>
        /// <exception cref="ArgumentException">Wrong Lean <see cref="Symbol"/></exception>
        public Symbol GetLeanSymbolFromOpenOrders(string brokerageSymbol, SecurityType securityType = SecurityType.Crypto, string market = Market.Bitstamp, DateTime expirationDate = default(DateTime),
            decimal strike = 0, OptionRight optionRight = OptionRight.Call)
        {
            if (market != Market.Bitstamp)
            {
        ...
        /// <summary>
        /// Get Bitstamp Websocket ticker for passed <see cref="Symbol"/>
        /// </summary>
        /// <param name="symbol">Lean <see cref="Symbol"/></param>
        ...
        /// <summary>
        /// Get Lean <see cref="Symbol"/> from Bitstamp Websocket ticker </summary>
        ...
        public Symbol GetSymbolFromWebsocket(string wsSymbol)
        {
            ...
            if (symbol.Equals(default(KeyValuePair<Symbol, SymbolProperties>)))
            {
                throw new ArgumentException($"Unknown symbol: {wsSymbol}/{SecurityType.Crypto}/{Market.Bitstamp}");
            }
        ...
        /// <summary>
        /// Convert Bitstamp Currency to Lean Currency
        /// </summary>
        /// <param name="marketCurrency">Bitstamp currency</param>
        ...
        public string ConvertCurrency(string marketCurrency)
        {
            ...
            {
                // Lean currency the same with Bitstamp one
