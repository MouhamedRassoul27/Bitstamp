    ...
    public partial class BitstampBrokerage
    {
        ...
        protected override void OnMessage(object sender, WebSocketMessage e)
        {
            ...
            try
            {
                ...
                {
                    var response = token.ToObject<BitstampBaseWsResponse>();
                    ...
                    {
                        OnMessage(new BrokerageMessageEvent(BrokerageMessageType.Information, 200, $"BitstampWS system status: {token["status"]}"));
                        return;
                    }

                    if (response.Event == "addOrderStatus")
                    {
                        var addOrder = token.ToObject<BitstampWsAddOrderResponse>();
        ...
        private void EmitOrderEvent(JToken update)
        {
            foreach (var trade in update.Children())
            {
                try
                {
                    ...
                    if (order.Properties is BitstampOrderProperties bitstampOrderProperties)
                    {
                        if (bitstampOrderProperties.FeeInBase)
                        {
                            feeCurrency = @base;
                        }
                    }
                    ...
                    {
                        var orderData = data.Value.ToObject<BitstampBaseWsResponse>();
                        ...
                        (
                            ...
                            order.Direction, 0, 0,
                            new OrderFee(new CashAmount(0m, feeCurrency)), $"Bitstamp Order Event {order.Direction}"
                        );
                    }
                    else
                    {
                        ...
                        {
                            var orderData = data.Value.ToObject<BitstampWsOpenOrder>();
                        ...
                        {
                            var orderData = data.Value.ToObject<BitstampOrderStatusEvent>();
                            fillQuantity = orderData.Vol_exec;
                            orderFee = new OrderFee(new CashAmount(orderData.Fee, feeCurrency));
                        ...
                        {
                            var orderData = data.Value.ToObject<BitstampTradeExecutionEvent>();
                        ...
                        (
                            ...
                            orderFee, $"Bitstamp Order Event {direction}"
                        );
                    }
        ...
        private JsonObject CreateBitstampOrder(Order order)
        {
            ...
            if (order.Properties is BitstampOrderProperties bitstampOrderProperties)
            {
                ...
                if (bitstampOrderProperties.PostOnly)
                {
                ...
                if (bitstampOrderProperties.FeeInQuote)
                {
                    propertiesList.Add("fciq");
                }

                if (bitstampOrderProperties.NoMarketPriceProtection)
                {
                ...
                if (bitstampOrderProperties.ConditionalOrder != null)
                {
                    if (bitstampOrderProperties.ConditionalOrder is MarketOrder)
                    {
                        throw new BrokerageException($"BitstampBrokerage.PlaceOrder: Conditional order type can't be Market. Specify other order type");
                    }
                    if (bitstampOrderProperties.ConditionalOrder is LimitOrder limitOrd)
                    {
                        parameters.Add("close[ordertype]", "limit");
                        parameters.Add("close[price]", limitOrd.LimitPrice.ToStringInvariant());
                    }
                    else if (bitstampOrderProperties.ConditionalOrder is StopLimitOrder stopLimitOrder)
                    {
                    ...
                    else if (bitstampOrderProperties.ConditionalOrder is StopMarketOrder stopOrder)
                    {
                        parameters.Add("close[ordertype]", "stop-loss");
                        parameters.Add("close[price]", stopOrder.StopPrice.ToStringInvariant());
                    }
                    else if (bitstampOrderProperties.ConditionalOrder is LimitIfTouchedOrder limitIfTouchedOrder)
                    {
