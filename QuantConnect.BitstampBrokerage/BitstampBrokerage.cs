...
{
    /// <summary>
    /// Bitstamp brokerage implementation
    /// </summary>
    [BrokerageFactory(typeof(BitstampBrokerageFactory))]
    public partial class BitstampBrokerage : BaseWebsocketsBrokerage, IDataQueueHandler
    {
        ...
        private BitstampBrokerageRateLimits _rateLimiter;

        private const int MaximumSymbolsPerConnection = 50;
        private const string _apiUrl = "https://www.bitstamp.net";
        private const string _wsUrl = "wss://ws.Bitstamp.com";
        private const string _wsAuthUrl = "wss://ws-auth.bitstamp.com";
        ...
        /// </summary>
        public BitstampBrokerage() : base("Bitstamp")
        {
        }
        ...
        public BitstampBrokerage(string apiKey, string apiSecret, string verificationTier, int orderBookDepth, IAlgorithm algorithm, IDataAggregator aggregator, LiveNodePacket job)
            : base("Bitstamp")
        {
        ...
        /// <param name="aggregator"><see cref="IDataAggregator"/> instance</param>
        /// <param name="job">Lean <see cref="LiveNodePacket"/></param>
        public BitstampBrokerage(IAlgorithm algorithm, IDataAggregator aggregator, LiveNodePacket job)
            :
            this(Config.Get("Bitstamp-api-key"),
                Config.Get("Bitstamp-api-secret"),
                Config.Get("Bitstamp-verification-tier"),
                Config.GetInt("Bitstamp-orderbook-depth", 10),
        ...
        /// <summary>
        /// Gets all Bitstamp orders not yet closed
        /// </summary>
        ...
        public override List<Order> GetOpenOrders()
        {
            ...
            {
                var bitstampOrder = item.Value.ToObject<BitstampOpenOrder>();
                Order order;
                var quantity = bitstampOrder.Vol;
                var symbol = _symbolMapper.GetLeanSymbolFromOpenOrders(bitstampOrder.Descr.Pair);
                var time = !string.IsNullOrEmpty(bitstampOrder.Opentm) ? Time.UnixTimeStampToDateTime(bitstampOrder.Opentm.ConvertInvariant<double>()) : DateTime.UtcNow;
                var brokerId = item.Name;
                
                var properties = new BitstampOrderProperties();

                if (bitstampOrder.Oflags.Contains("post"))
                {
                    properties.PostOnly = true;
                }
                if (bitstampOrder.Oflags.Contains("fcib"))
                {
                    properties.FeeInBase = true;
                }
                if (bitstampOrder.Oflags.Contains("fciq"))
                {
                    properties.FeeInQuote = true;
                }
                if (bitstampOrder.Oflags.Contains("nompp"))
                {
                    properties.NoMarketPriceProtection = true;
                }

                switch (bitstampOrder.Descr.OrderType.LazyToUpper())
                {
                    case "MARKET":
                        order = new MarketOrder(symbol, quantity, time, properties: properties);
                        break;
                    case "LIMIT":
                        var limPrice = bitstampOrder.Descr.Price;
                        ...
                        break;
                    case "STOP-LOSS":
                        var stopPrice = bitstampOrder.Descr.Price;
                        ...
                        break;
                    case "TAKE-PROFIT":
                        var tpPrice = bitstampOrder.Descr.Price;
                    ...
                    case "STOP-LOSS-LIMIT":
                        var stpPrice = bitstampOrder.Descr.Price;
                        var limitPrice = bitstampOrder.Descr.Price2;
                    ...
                    case "TAKE-PROFIT-LIMIT":
                        var takePrice = bitstampOrder.Descr.Price;
                        var lmtPrice = bitstampOrder.Descr.Price2;
                ...
                order.Status = GetOrderStatus(bitstampOrder.Status);
        ...
        public override List<Holding> GetAccountHoldings()
        {
            ...
            {
                var BitstampPosition = balance.Value.ToObject<BitstampOpenPosition>();
                var holding = new Holding
                {
                    Symbol = _symbolMapper.GetLeanSymbol(BitstampPosition.Pair),
                    Quantity = BitstampPosition.Vol,
                    UnrealizedPnL = BitstampPosition.Net,
                    MarketValue = BitstampPosition.Value,
                };

                holding.AveragePrice = BitstampPosition.Cost / holding.Quantity;
                CurrencyPairUtil.DecomposeCurrencyPair(holding.Symbol, out _, out var quote);
                holding.CurrencySymbol = quote;
                if (BitstampPosition.Type == "sell")
                {
                    holding.Quantity *= -1;
                }
        ...
        public override List<CashAmount> GetCashBalance()
        {
            ...
            if (_algorithm.BrokerageModel.AccountType == AccountType.Margin)
            {
                ...
                for (var i = 0; i < holdings.Count; i++)
                {
                    ...
                    // Bitstamp margin balances logic
        ...
        public override bool PlaceOrder(Order order)
        {
            ...
            var parameters = CreateBitstampOrder(order);
        ...
        {
            throw new NotSupportedException("BitstampBrokerage.UpdateOrder: Order update not supported. Please cancel and re-create.");
        }
        ...
        {
            Log.Trace("BitstampBrokerage.CancelOrder(): {0}", order);

            if (!order.BrokerId.Any())
            {
                // we need the brokerage order id in order to perform a cancellation
                Log.Trace("BitstampBrokerage.CancelOrder(): Unable to cancel order without BrokerId.");
                return false;
            }
        ...
        public override IEnumerable<BaseData> GetHistory(HistoryRequest request)
        {
            ...
            if (request.Resolution != Resolution.Tick && request.StartTimeUtc < DateTime.UtcNow - period * 720)
            {
                OnMessage(new BrokerageMessageEvent(BrokerageMessageType.Warning, "NotFullHistoryWarning",
                    $"Bitstamp return max 720 TradeBars in history request. Now it will return TradeBars starting from {DateTime.UtcNow - period * 720}"));
            }
        ...
        protected void Initialize(string apiKey, string apiSecret, string verificationTier, int orderBookDepth, IAlgorithm algorithm, IDataAggregator aggregator, LiveNodePacket job)
        {
            ...
            base.Initialize(_wsAuthUrl, new BitstampWebSocketWrapper(null), new RestClient(_apiUrl), apiKey, apiSecret);
            ...
            _rateLimiter = new BitstampBrokerageRateLimits(verificationTier);
            ...
            SubscriptionManager = new BrokerageMultiWebSocketSubscriptionManager(
                ...
                0,
                null,
                () => new BitstampWebSocketWrapper(null),
                Subscribe,
                Unsubscribe,
                OnDataMessage,
                TimeSpan.Zero,
        ...
        private IEnumerable<BaseData> GetOhlcBars(HistoryRequest request, string marketSymbol, TimeSpan period)
        {
            ...
            var url = $"/api/v2/ohlc/{marketSymbol}/?step={resolution}";
            ...
            while (end - start >= resolutionInMs)
            {
                ...
                var candlesList = token["result"][marketSymbol].ToObject<List<BitstampCandle>>();
                ...
                if (candlesList.Count > 0)
                {
                    ...
                    if (Log.DebuggingEnabled)
                    {
                        ...
                        Log.Debug($"BitstampBrokerage.GetOhlcHistory(): Received [{marketSymbol}] data for time period from {windowStartTime.ToStringInvariant()} to {windowEndTime.ToStringInvariant()}..");
                    }
                    ...
                    {
                        if (candlesList[i].Time > end) // no "to" param in Bitstamp and it returns just 1000 candles since start timestamp
                        {
                            yield break;
                        }
        ...
        {
            var url = $"/api/v2/ticker/{marketSymbol}";
            ...
            while (end - start > 1) // allow 1 sec difference because of rounding from decimal to long
            {
                ...
                var tradesList = token["result"][marketSymbol].ToObject<List<BitstampTrade>>();
                ...
                if (tradesList.Count > 0)
                {
                    ...
                    if (Log.DebuggingEnabled)
                    {
                        ...
                        Log.Debug($"BitstampBrokerage.GetTradeHistory(): Received [{marketSymbol}] data for time period from {windowStartTime.ToStringInvariant()} to {windowEndTime.ToStringInvariant()}..");
                    }
                    ...
                    {
                        if (tradesList[i].Time > end) // no "to" param in Bitstamp and it returns just 1000 candles since start timestamp
                        {
                            yield break;
                        }
        ...
        private JToken MakeRequest(string query, string methodCaller, IDictionary<string, object> requestBody = null, Method method = Method.GET, bool isPrivate = false)
        {
            ...
            if (isPrivate)
            {
                ...
                requestBody.Add("key", Config.Get("Bitstamp-api-key") );
                requestBody.Add("signature", GetSignature(nonce));
            ...
            {
                throw new Exception($"BitstampBrokerage.{methodCaller}: request failed: [{(int) response.StatusCode}] {response.StatusDescription}, Content: {response.Content}, ErrorMessage: {response.ErrorMessage}");
            }
            
            if (response.StatusCode == HttpStatusCode.OK && token["error"].HasValues)
            {
                throw new Exception($"BitstampBrokerage.{methodCaller}: request failed: {token["error"]}");
            }
